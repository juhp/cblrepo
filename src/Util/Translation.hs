                ...
                libBuildFunction = text "build() {" <>
                    nest 4 (empty <$>
                        ...
                        maybe empty (\ _ ->
                            ...
                            cabalPatchFile <$>
                        maybe empty (\ _ ->
                            text $ "patch -p4 < ${srcdir}/build.patch")
                            buildPatchFile <$>
...
addPatches patchDir ap = let
        ...
        buildPatchFn = patchDir </> hkgName <.> "build"
    in do
        ...
        let sources' = shVarAppendValue sources
                (ShArray $ catMaybes [maybe Nothing (const $ Just "cabal.patch") cabalPatch, maybe Nothing (const $ Just "build.patch") buildPatch])
        return ap
...
copyPatches destDir ap = let
    ...
    in do
        ...
        maybe (return ()) (\ fn -> copyFile fn (destDir </> "build.patch")) buildPatch
